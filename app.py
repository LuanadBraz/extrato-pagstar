...
if submitted:
    try:
        ...
        if resultado == "codigo":
            ...
            st.warning("🔐 O site solicitou código de segurança enviado por e-mail. Digite abaixo para continuar.")
            st.info("📧 Verifique seu e-mail e insira o código manualmente para que o sistema continue.")
        else:
            data, nome = resultado
...
if st.session_state.get("aguardando_codigo"):
    ...
    if confirmar:
        try:
            ...
            st.success("✅ Extrato gerado com sucesso após confirmação!")
            st.download_button("⬇️ Baixar extrato", data, file_name=nome)
